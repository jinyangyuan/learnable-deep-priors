...
class FC(nn.Module):
    ...
    def __init__(self, hidden_list, num_features_in, num_features_out, activation, last_activation):
        super(FC, self).__init__()
        layers = []
        for num_features in hidden_list:
            layers = layers + self.create_layers(num_features_in, num_features, activation)
            num_features_in = num_features
        layers = layers + self.create_layers(num_features_in, num_features_out, activation, last_activation)
        self.net = nn.Sequential(*layers)

    @staticmethod
    def create_layers(num_features_in, num_features_out, activation, use_activation=True):
