...
def test_model(args, net, dataloader):
    ...
    for model_id in range(args.num_tests):
        result_list = {key: [] for key in ['pres', 'segre', 'recon_objects', 'recon_scene', 'back', 'll_mixture',
                                           'll_single']}
        for images in dataloader['test']:
            ...
            segre = net.module.compute_prob_prior(result['shp'])
            log_segre = net.module.compute_log_prob_prior(result['logits_shp'])
            pres = segre.view(*segre.shape[:-2], -1)
            pres = torch.eq(pres, pres.max(0, keepdim=True).values).any(-1).type(torch.float32)[:-1]
            ...
            ll_mixture, ll_single = net.module.compute_log_likelihood(images, result, log_segre, recon_scene)
        ...
        with h5py.File(os.path.join(args.folder, args.file_result_base.format(model_id)), 'w') as f:
            for key, val in result_list.items():
                f.create_dataset(key, data=torch.cat(val).numpy(), compression='gzip', chunks=True)
