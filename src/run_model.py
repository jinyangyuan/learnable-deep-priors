...
def test_model(config, data_loaders, net):
    ...
    def compute_layer_mse(layers_list, apc_list, shp_list, order_list):
        bck_rgb, objects_rgb, objects_a = layers_list[:, :1, :-1], layers_list[:, 1:, :-1], layers_list[:, 1:, -1]
        ...
        target_obj = objects_rgb * objects_a[:, :, None] + bck_rgb * (1 - objects_a[:, :, None])
        sq_diffs = np.square(recon_obj - target_obj).mean(-3)
